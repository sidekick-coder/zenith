...
const mailerId = computed(() => route.params.id as string)

const mailer = ref<MailerConfig>()
...
const tabs = [
    {
        id: 'config',
        label: $t('Config'),
        component: defineAsyncComponent(() => import('#client/components/MailerConfig.vue')),
    },
    // {
    //     id: 'entries',
    //     label: $t('Entries'),
...
async function load() {
    ...
    if (error) {
        console.error('Failed to load mailer:', error)
        loading.value = false
        return
    }
...
const onSubmit = handleSubmit(async (data) => {
    ...
    const [error] = await $fetch.try(`/api/mailers/${mailerId.value}`, {
        method: 'PUT',
        data
    })
