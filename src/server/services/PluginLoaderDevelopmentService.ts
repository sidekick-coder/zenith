import fs from 'fs'
import { clientPath } from '@sidekick-coder/zenith-kit/server/utils/basePath'
...
export default class PluginLoaderDevelopmentService extends PluginLoaderService {
    ...
    public async loadClientPLugin(): Promise<void> {
        ...
        // clean old plugin entries
        const files = await fs.promises.readdir(clientPath('.plugins'))

        const pluginFiles = files.filter(file => file.endsWith('.ts'))

        for (const file of pluginFiles) {
            await fs.promises.unlink(clientPath('.plugins', file))
        }
        ...
        for (const plugin of plugins) {
            ...
            const filename = clientPath('.plugins', `${plugin.id}.ts`)

            let content = '// This file is auto-generated by PluginLoader, do not edit\n\n'
